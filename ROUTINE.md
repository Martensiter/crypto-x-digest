...
旧 GitHub Actions cron (`fetch-twitter-timeline.yml`、削除済み) に代わり、**Claude Code on the web
...
## 仕組み（旧 GHA fetch-twitter-timeline.yml と同等のパイプライン）
...
- 旧 `fetch-twitter-timeline.yml` は**削除済み**（2026-07-18。cron 停止のまま dispatch
  フォールバックとして残置していたのを整理。復元は git 履歴から）。
- 手動の単発実行は、セッションで CLAUDE.md のパイプライン（`fetch_timeline.py` →
  filter → cluster → digest 生成）をそのまま実行する。
- トークン (`X_AUTH_TOKEN` / `X_CT0`) は Routine env で管理し、最新に保つこと。
